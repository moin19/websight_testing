        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    result = cursor.callfunc("fn_update_sales",str, [product_code.product_code,total_quantity])
                    print(result)
...
class manage_image_view(TemplateView):
    template_name = 'news/image.html'
    context={}
...
def show_image(request,phone_number):
    if not request.user.is_authenticated:
        return render(request, 'news/employee-login.html')
    template_name = 'news/show-image.html'
    context={}
    data=dict()
    
    try:
        sign_data = Client_Photo_Sign.objects.get(client_phone=phone_number, document_type='S')
    ...
    try:
        image_data = Client_Photo_Sign.objects.get(client_phone=phone_number, document_type='P')
    ...
    data['html_form'] = data['html_form'].replace('client_photo',data['client_photo'])
    data['html_form'] = data['html_form'].replace('client_signature',data['signature_card'])
